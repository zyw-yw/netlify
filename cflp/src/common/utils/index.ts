...
export const coordinatesTransfer = (totalMarkers: any) => {
  const orders = totalMarkers?.orders?.detail.map((user: any) => {
    return {
...
export const getHistories = async (email: string, productType?: string) => {
  if (productType) {
    // company查询历史
  } else {
    // client全量加载
...
export { getPathDirections, getUserInfo } from "./server";
